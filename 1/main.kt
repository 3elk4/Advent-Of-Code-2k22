...
class Elf(calories : ArrayList<Int>) : Comparable<Elf> { 
  val totalCalories = calories.sum()

  override fun compareTo(other: Elf): Int { // desc
    return other.totalCalories - totalCalories
  }
}

fun elfWithMaxCalories(elves : ArrayList<Elf>) : Elf  {
  return elves.maxByOrNull { it.totalCalories }!!
}

fun topElvesWithMaxCalories(n : Int, elves : ArrayList<Elf>) : List<Elf> {
  elves.sort()
  return elves.take(n)
}
...
fun main() {
  ...
  for(line in lines) {
    ...
    tempArr.add(line.toInt())
  }
  
  val result = elfWithMaxCalories(elves)
  println("Elf with ${result.totalCalories} calories.")

  val result2 = topElvesWithMaxCalories(3, elves)
  result2.forEach { elf -> println(elf.totalCalories)}
  val total = result2.sumBy { it.totalCalories }
  println("Elves with ${total} calories.")
}
